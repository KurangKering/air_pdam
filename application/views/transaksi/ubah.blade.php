		...
		<div class="card">
			...
			<div class="card-body">
				<form id="form-transaksi" enctype="multipart/form-data" method="GET">
					...
					<input type="hidden" name="input-transaksi_id" value="{{ $data['last_transaction']['id'] }}">
					<input type="hidden" name="input-periode_bulan" value="{{ $data['last_transaction']['bulan'] }}">
					<input type="hidden" name="input-periode_tahun" value="{{ $data['last_transaction']['tahun'] }}">
					<div class="form-group">
						<label class="form-label">Nama Perusahaan</label>
						<input type="text" class="form-control" value="{{ $data['data_perusahaan']['nama_perusahaan'] }}" disabled>
					</div>
					<div class="form-group">
						<label class="form-label">Periode Bulan</label>
						<input type="text" class="form-control" name="" value="{{ $data['last_transaction']['bulan_huruf'] }}" disabled>
					</div>
					<div class="form-group">
						<label class="form-label">Periode Tahun</label>
						<input type="text" class="form-control" name="" value="{{ $data['last_transaction']['tahun'] }}" disabled>
					</div>
					<div class="form-group">
						<label class="form-label">Status Transaksi</label>
						<select name="input-status" id="status" class="form-control">
							@foreach ($data['daftar_status'] as $k => $v)
							<option 

							@if ($k == $data['last_transaction']['status'])
							selected 
							@endif
...
<script>
	...
	$(function() {
		...
		axios.post('{{ base_url('transaksi/getDataForUbahMenu') }}', data)
		.then(res => {
		...
		$("#btn-submit").click(function(event) {
			...
			axios.post('{{ base_url('transaksi/update') }}', data)
			.then(res => {
				if (!res.data.success) 
				{
					swal({
