...
class Transaksi extends MY_Controller
{
	...
	public function tambah()
	{
		...
		if ($client_id == NULL)
			return show_404();
		$client = $this->M_Client->where('id', $client_id)->first();
		if (!$client)
			return show_404();
	...
	private function insertOutputJson()
	{
		...
		$client_id = $post_transaction['client_id'];
		$periode_filename = "{$post_tahun}-{$post_bulan}";
		...
		$file_pembayaran = $this->uploadmanager->uploadPembayaran('file_pembayaran', $filename);
